...
<div class="my-3 px-5 p-3 bg-body rounded shadow-sm">
    ...
    <div class="d-flex justify-content-between mt-4" >
        ...
        <!-- TOMBOL TAMBAH DATA -->
        <div class="pl-4 pb-3">
            <a href="{{ url('pegawai/create') }}" class="btn btn-outline-primary">+ Tambah Data</a>
        </div>

        <!-- FORM PENCARIAN -->
        <div class="pb-2">
            <form class="d-flex" action="{{ url('pegawai') }}" method="get" autocomplete="off">
        ...
        <?php $i = $data->firstitem() ?>
            @foreach ($data as $item)
                <tr>
                    ...
                    <td>
                        <a href="{{ url('pegawai/'.$item->id.'/edit')  }}" class="btn btn-warning btn-sm">Ubah</a>
                        <form onsubmit="return confirm('Yakin menghapus data pegawai?')" class="d-inline" action="{{ url('pegawai/'.$item->id) }}" method="post">
                            @csrf
                            @method('DELETE')
